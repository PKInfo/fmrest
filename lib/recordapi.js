...
class RecordAPI {
    ...
    constructor(Auth) {
        this.Auth = Auth;
        this.url = `https://${this.Auth.getHost()}/fmi/data/vLatest/databases/${this.Auth.getDatabase()}/layouts/${this.Auth.getLayout()}/records`;
    }
     ...
     */
    create(json) {
        if (json === undefined) json = { fieldData: {} };
        else json = { fieldData: json };
    ...
    edit(id, json, modId) {
        if (json === undefined) json = { fieldData: {} };
            ...
            json = { fieldData: json };
            if (modId !== undefined) json.modId = modId;
        }
    ...
    createPortalQueries(portals) {
        ...
        let options = portals.map((el) => {
            let obj = el.toObj();
            let options = '';
            if (obj.offset !== null) options += `&_offset.${obj.name}=${obj.offset}`;
            if (obj.limit !== null) options += `&_limit.${obj.name}=${obj.limit}`;
            return options;
        });
    ...
    getAll({ limit, offset, sorts, portals } = {}) {
        ...
        if (offset !== undefined) params.push(`_offset=${offset}`);
        ...
        if (params.length > 0) url = `${this.url}?${params.join('&')}`;
        else url = `${this.url}`;
