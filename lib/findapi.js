...
class FindAPI {
    ...
    constructor(Auth) {
        this.Auth = Auth;
        this.url = `https://${this.Auth.getHost()}/fmi/data/vLatest/databases/${this.Auth.getDatabase()}/layouts/${this.Auth.getLayout()}/_find`;
    }
    ...
    exec({ requests, sorts, offset, limit, portals } = {}) {
        ...
        if (offset !== undefined) body.offset = offset.toString();

        if (limit !== undefined) body.limit = limit.toString();
